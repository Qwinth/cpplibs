...
#ifdef _WIN32
        setsockopt(s, level, optname, reinterpret_cast<const char*>(&optval), sizeof((char*)optval));
#elif __linux__
        setsockopt(s, level, optname, &optval, sizeof(int));
#endif
    }

    ret sgetsockopt(int level, int optname) {
        ret r;
#ifdef _WIN32
        int error_code_size = sizeof(r.error);
        r.retval = getsockopt(s, SOL_SOCKET, SO_ERROR, (char*)&r.error, &error_code_size);
#elif __linux__
        socklen_t error_code_size = sizeof(r.error);
        r.retval = getsockopt(s, SOL_SOCKET, SO_ERROR, &r.error, &error_code_size);
#endif
